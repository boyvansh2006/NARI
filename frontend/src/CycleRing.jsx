...
export default function CycleRing({ lengths = [] }) {
  ...
  const colors = ["#1E405C", "#347BA8", "#4D7D9A", "#E07A6F", "#275375", "#5295BA"];
  const darkFills = new Set(["#1E405C", "#275375", "#347BA8", "#E07A6F"]);

  return (
    <svg viewBox={`0 0 ${size} ${size}`} width="100%" style={{ maxWidth: 240, display: "block", margin: "8px auto" }}>
      <circle cx={center} cy={center} r={maxR + 14} fill="#EDF3F5" />
      {lengths.map((len, i) => {
        ...
        const textColor = darkFills.has(fill) ? "#FFFFFF" : "#0D1D2C";
        return (
          <g key={i}>
      ...
      <circle cx={center} cy={center} r={26} fill="#1E405C" />
